...
- **Payment data**: billing address, last four digits of payment card (full card data
  processed by our PCI-DSS certified payment processor, Example Payments Corp.).
- **Communications data**: customer support tickets, email correspondence.

We collect personal information directly from users upon account registration and indirectly
through automated tracking technologies on our platform.
...
- **Consent**: Users must expressly consent to the collection and use of their personal
  information for marketing communications by checking an opt-in box at registration.
- **Contractual necessity**: Processing of account data and payment data is necessary to
  fulfill the user's service agreement.
- **Legitimate interests**: Usage analytics and platform security monitoring are conducted
  under our legitimate interests, subject to users' right to object.

Users may withdraw consent to marketing communications at any time by clicking the
unsubscribe link in any marketing email or by contacting privacy@example.org.
...
- **Stacklayer Cloud Inc.**: hosts our platform infrastructure and database servers in Canada;
  bound by a data processing agreement prohibiting unauthorized access.
- **Insightful Analytics Ltd.**: receives pseudonymised usage data for aggregate analytics;
  data is transferred via encrypted API with no personal identifiers.
- **Meridian Support Systems Inc.**: receives communications data for customer support
  ticketing; access limited to support staff only.

Personal information is not transferred outside Canada except to Stacklayer Cloud Inc.'s
disaster recovery facilities in the United States, which are subject to equivalent contractual
protections under cross-border data transfer agreements.
...
Canada regarding our privacy practices.
