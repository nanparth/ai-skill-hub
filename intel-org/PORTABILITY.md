...
- `agents/intel-org-researcher.md` — researcher process (subagent template or inline checklist)
- `intel-org-readme.md` — user guide and technical reference
- `PORTABILITY.md` — dependency and copy boundary

## Required When Copying

Copy the whole `intel-org/` folder into the host assistant's skills folder (for example `~/.claude/skills/intel-org/` in Claude Code; equivalent locations in other assistants).
...
- agent-browser CLI (https://github.com/vercel-labs/agent-browser): scraping JS-rendered sites, screenshots, logo capture, network-request inspection. Fallback: direct page fetch; visual assets skipped and recorded as gap flags.
...
- Registry examples are Canadian (Corporations Canada, SEDAR+); substitute your jurisdiction's equivalents.
- Shell examples are POSIX-flavoured (`mkdir -p`, `curl`); adjust for other shells.
...
- Browser screenshots, logo capture, and network inspection are opt-in through the optional browser tool; without it, the skill records those assets as skipped.
